...
A single-page Kanban board for tracking IT project tasks across four columns: Backlog, In Progress, Blocked and Done. It's written in plain HTML, CSS and JavaScript, with each version of the board in a single file, no build step and no dependencies.
...
**Live demo:** https://sam50123.github.io/kanban4/ (redesign: https://sam50123.github.io/kanban4/redesign.html)
...
The repo has two standalone versions of the board with the same behaviour: `index.html` (the original design) and `redesign.html` (a light header, a status donut showing % complete, and restyled columns and cards).
...
- IT support popup after 10 seconds on the page, with the IT Support Hotline number (shown once per page load)
- Responsive layout, visible focus styles, screen-reader labels and support for reduced motion

## Run locally

No install or server is needed. Clone the repo and open `index.html` (or `redesign.html`) in a browser:
...
New-task notifications go to the address set in `FORMSUBMIT_ENDPOINT` at the top of the `<script>` block in `index.html` and in `redesign.html` (change both):
...
- Only `index.html` and `redesign.html` are published. If you add files the site needs, copy them into `_site` in the workflow's "Assemble site" step.

## Project structure

```
index.html                    # the whole app: markup, one <style>, one <script>
redesign.html                 # the redesigned board, same single-file structure
docs/screenshot.png           # README screenshot (captured with the Playwright MCP server)
.github/workflows/pages.yml   # GitHub Pages deploy
.claude/commands/publish.md   # Claude Code /publish command (scan → document → screenshot → push → Pages)
.mcp.json                     # project-level MCP servers (Playwright)
